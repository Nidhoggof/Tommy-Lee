...
from models.LSTMWideDeep import LSTMWideDeep
...
def main():
    ...
    init_parser.add_argument('--model_name', type=str, default='LSTMWideDeep',
                             help='Choose model to run.')#PreHashWideDeep # LSTMWideDeep
    ...
    log_file_name = [str(init_args.rank) + str(init_args.drop_neg),
                     init_args.model_name, origin_args.dataset]
    ...
    np.save(args.result_file.replace('.npy', '_train.npy'), train_result)
    np.save(args.result_file.replace('.npy', '_valid.npy'), validation_result)
    np.save(args.result_file.replace('.npy', '_test.npy'), test_result)
