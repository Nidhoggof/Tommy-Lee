...
class WideDeep(DeepModel):
    ...
    def predict(self, feed_dict):
        ...
        nonzero_embeddings = self.feature_embeddings(torch.tensor(feed_dict[X]).to(torch.int64))
        ...
        cross_bias = self.cross_bias(torch.tensor(feed_dict[X]).to(torch.int64)).sum(dim=1).view([-1])
